...
const analyticsData = [
  ...
  const findLongestSessionUser = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      const person = data[i];
      if (person.avgSessionDuration > maximum) {
        maximum = person.avgSessionDuration;
  ...
  const formatSessions = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      const user = data[i];
      let format = `${user.name}: ${user.totalSessions} sessions`;
      mapping.push(format);
    };
    return mapping;
  };
  ...
  const getActiveUsers = (data) => {
    ...
    const highnames = data
      .filter(data => data.totalSessions >= 5)
      .map(data => data.name);
    return highnames
  };
  ...
  const getTotalSessions = (data) => {
    ...
    // Hint: Accumulate user.totalSessions
     ...
     for (let i = 0; i < data.length; i++) {
      let user = data[i];
      totalSum = totalSum + user.totalSessions;
     };
     return totalSum;
  };
